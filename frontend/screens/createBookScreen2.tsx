  ...
  const toggleKeywordSelection = (keywordName: string) => {
    if (selectedKeywordNames.includes(keywordName)) {
      ...
      setSelectedKeywordNames(
        selectedKeywordNames.filter(name => name !== keywordName),
      );
    } else {
  ...
  const getGenre = async () => {
    try {
      const response = await axios.get(
        `http://${REACT_NATIVE_BACKEND_IP}:3000/api/keywordList`,
        {
          params: {
            genre_id: genreId,
          },
      ...
      <ScrollView
        ...
        <View
          ...
          {keyWordsInput.map((keyWord, index) => {
            const isSelected = selectedKeywordNames.includes(
              ...
              >
                <Text
                  style={[
                    ...
                    {fontSize: 14, color: isSelected ? 'white' : 'black'},
                  ]}>
                  {keyWord.keyword_name}
                </Text>
              </TouchableOpacity>
