...
import {
  ...
  createImages,
  generateStory,
} from '../local_modules/openaiUtill.js';
    ...
    Book.create(
      ...
      async (err, book_id) => {
        if (err) {
          return res
            .status(500)
            .json({error: 'Failed to create book', details: err});
        }
        ...
        try {
          ...
          for (let index = 0; index < paragraphs.length; index++) {
            ...
            try {
              console.log(
                `Processing paragraph ${index + 1} of ${paragraphs.length}`,
              );
          ...
          res
            .status(200)
            .json({message: 'Book and paragraphs created successfully'});
        } catch (error) {
          console.error('Error processing paragraphs:', error);
          res
            .status(500)
            .json({error: 'Failed to process some paragraphs', details: error});
        }
      },
    );
  } catch (error) {
...
router.get('/book/:book_id/full', async (req, res) => {
  ...
  try {
    ...
    const [book, paragraphs] = await Promise.all([
      bookPromise,
      paragraphPromise,
    ]);
    ...
    if (paragraphs.length === 0) {
      return res
        .status(404)
        .json({book, error: 'No paragraphs found for the book'});
    }
    ...
    res
      .status(500)
      .json({error: 'Failed to fetch book and paragraphs', details: error});
  }
});
...
router.delete('/book', async (req, res) => {
  ...
  try {
    ...
    const deletePromises = paths.map(async path => {
      ...
      if (path.image_path) {
        try {
          ...
          console.error(
            `Failed to delete image file ${path.image_path}:`,
            error,
          );
        }
      }
    ...
    res
      .status(200)
      .json({message: 'Book and related Paragraphs deleted successfully'});
  } catch (error) {
